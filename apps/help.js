...
const reloadConfig = () => {
  try {
    // 使用配置
    const H1 = Cfg.getConfig('help_config').main_title || 'iloli-plugin';
    const H2 = Cfg.getConfig('help_config').sub_title || '默认副标题';
    const columns = Cfg.getConfig('help_config').columns || 3;
    const deviceScaleFactor = Cfg.getConfig('help_config').deviceScaleFactor || 0.9;
    const yiyan = Cfg.getConfig('help_config').default_hitokoto || '种自己的花，爱自己的宇宙';
    const bg = Cfg.getConfig('help_config').background_image_url || 'none';
...
const preRenderHelp = async () => {
  ...
  logger.mark(`\x1b[32mHelp-Plugin-Lite预渲染完成！\x1b[0m`);
};

logger.mark( )
logger.mark(`\x1b[32m----------ヾ(✿ﾟ▽ﾟ)ノ----------\x1b[0m`)
logger.mark(`\x1b[32m   Help-Plugin-Lite载入成功！\x1b[0m`)
logger.mark(`\x1b[32m      Created By Jiaozi\x1b[0m`)
logger.mark(`\x1b[32m      交流群：983299064\x1b[0m`)
...
export class HelpPlugin extends plugin {
  constructor() {
    super({
      ...
      rule: [
        {
          reg: '^(#|/)?(云崽|云宝|全部)?(i|萝莉)(帮助|菜单|help|功能|说明|指令|使用说明|命令)(列表)?$',
          fnc: 'showHelp'
        },
        {
          reg: '^(#|/)?(刷新|重载|重置)(云崽|云宝|全部)?(i|萝莉)(帮助|菜单|help|功能|说明|指令|使用说明|命令)(列表)?$',
          fnc: 'refreshHelp'
        }
      ]
    });
